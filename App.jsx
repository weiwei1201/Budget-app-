...
const STORAGE_KEY = "budget-tracker-v14";
...
export default function App(){
  ...
  function openEditGoal(g){
    setEditGoal({id:g.id,form:{name:g.name,amount:String(g.amount),defaultDeposit:g.defaultDeposit?String(g.defaultDeposit):"",category:g.category||"other"}});
    closeKebab();setModal("edit-goal");
  }

  function submitEditGoal(){
    if(!editGoal)return;
    update(d=>({...d,goals:d.goals.map(g=>g.id!==editGoal.id?g:{
      ...g,name:editGoal.form.name.trim()||g.name,
      amount:Number(editGoal.form.amount)||g.amount,
      defaultDeposit:editGoal.form.defaultDeposit?Number(editGoal.form.defaultDeposit):null,
      category:editGoal.form.category||g.category,
    })}));
              ...
              <div style={{margin:"10px 12px 0",background:C.card,borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden"}}>
                ...
                {dayGroups.length===0
                  ...
                  :dayGroups.map(({groupId,lead})=>{
                    ...
                    return(
                      <div key={groupId} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 14px",borderBottom:`1px solid ${C.border}44`}}>
                        ...
                        <div style={{fontWeight:700,color:amtColor,fontSize:13,flexShrink:0}}>{amtSign}{fmt(lead.amount)}</div>
                      </div>
                    );
                  })
                }
          ...
          {savingsSub==="alloc"&&(
            <div>
              ...
              {allocGroups.map(({groupId,lead,allocatedAmt})=>{
                const isRedeem    =lead.type==="goal_redeem";
                const isDeposit   =lead.type==="savings_deposit"; // came from real account
                const isFromUnalloc=lead.type==="goal_deposit";   // from unallocated
                // For savings_deposit: find _out sibling for from-account
                const depOutTx=isDeposit
                  ?data.transactions.find(t=>t.groupId===groupId&&t.type==="transfer_out")
                  :null;
                const fromAcc=isDeposit?getAcc(depOutTx?.account||""):getAcc(lead.account);
                const iconEl=isRedeem?"−":isDeposit?"⭐":"🔄";
                const iconColor=isRedeem?"#c0392b":isDeposit?"#2d7a4f":C.accent;
                const goalName=isRedeem?(data.goals.find(g=>g.id===lead.goalId)?.name||"目標"):null;
                const dispAmt=isRedeem?lead.amount:isDeposit?lead.amount:allocatedAmt;
                return(
                  <div key={groupId} style={{...S.txRow,background:C.card}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {isFromUnalloc&&<div style={{fontWeight:600,fontSize:13}}>未分配 → 已分配<span style={{color:C.accent,marginLeft:5}}>{fmt(allocatedAmt)}</span></div>}
                      <div style={{fontSize:11,color:C.muted}}>
                        {lead.date} · {fromAcc.icon} {fromAcc.name}
                      </div>
                    </div>
                    <div style={{fontWeight:700,color:isRedeem?"#c0392b":isFromUnalloc?C.accent:"#2d7a4f",fontSize:14,flexShrink:0}}>
                      {isRedeem?"−":"+"}{fmt(dispAmt)}
                    </div>
                  </div>
                );
              })}

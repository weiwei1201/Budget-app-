...
export default function App(){
  ...
  function submitEditTx(){
    ...
    update(d=>{
      ...
      if(diff!==0){
        group.forEach(tx=>{
          ...
          if(["expense","withdraw_out","transfer_out"].includes(tx.type)){
            ...
            // savings_deposit: credit savings unallocated by diff
            savUnalloc+=diff;
          }
        });
      }
      ...
      return{...d,accounts:accs,transactions,savingsUnallocated:Math.max(0,savUnalloc),savingsTotalAllocated:Math.max(0,savAlloc)};
    });
  ...
  function submitEditAlloc(){
    ...
    update(d=>{
      ...
      if(lead.type==="savings_deposit"){
        // Find the transfer_out sibling to get the real source account
        const outTx=d.transactions.find(t=>t.groupId===editAlloc.groupId&&t.type==="transfer_out");
        const srcId=outTx?.account;
        const accs=d.accounts.map(a=>{
          if(a.id!==srcId) return a;
          // source account: more deposit = more deducted, less deposit = refund
          return {...a,balance:Number(a.balance)-diff};
        });
        // savings unallocated increases/decreases by diff (net of any allocations)
        savUnalloc+=diff;
  ...
  function deleteAllocGroup(groupId,lead){
    ...
    update(d=>{
      ...
      let goals=[...d.goals.map(g=>({...g}))];
      if(lead.type==="savings_deposit"){
        // Find transfer_out sibling to get real source account
        const outTx=d.transactions.find(t=>t.groupId===groupId&&t.type==="transfer_out");
        const srcId=outTx?.account;
        const accs=d.accounts.map(a=>a.id!==srcId?a:{...a,balance:Number(a.balance)+amt});
    ...
    const [cy,cm]=calYm.split("-").map(Number);
        ...
        return(
          <div style={{paddingBottom:20}}>
            ...
            {/* -- Below calendar: day panel or monthly view -- */}
            <div style={{padding:"8px 12px 0"}}>

              {/* Monthly stats — always visible */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>
                <div style={{background:C.card,borderRadius:10,padding:"8px 10px",border:`1px solid ${C.border}`}}>
                  <div style={{fontSize:10,color:C.muted}}>收入</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#2d7a4f"}}>{fmt(mInc)}</div>
                </div>
                <div style={{background:C.card,borderRadius:10,padding:"8px 10px",border:`1px solid ${C.border}`}}>
                  <div style={{fontSize:10,color:C.muted}}>支出</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#c0392b"}}>{fmt(mExp)}</div>
                </div>
                <div style={{background:C.card,borderRadius:10,padding:"8px 10px",border:`1px solid ${C.border}`}}>
                  <div style={{fontSize:10,color:C.muted}}>存入儲蓄</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#2d7a4f"}}>{fmt(mSav)}</div>
                </div>
              </div>

              {calDay?(
                /* -- Day selected: show day ledger only -- */
                <div style={{background:C.card,borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:10}}>
                  <div style={{padding:"9px 14px 8px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontWeight:700,fontSize:13,color:C.text}}>
                      {calDay.slice(5).replace("-","月")}日 流水帳
                    </span>
                    <button onClick={()=>setCalDay(null)}
                      style={{background:"none",border:"none",color:C.muted,fontSize:14,cursor:"pointer",padding:"2px 6px"}}>
                      ✕ 返回月份
                    </button>
                  </div>
                  {dayGroups.length===0
                ...
                /* -- No day selected: monthly ledger + pie -- */
                <>
                  {/* Monthly ledger */}
                  <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>
                    {ymLabel(calYm)} 流水帳
                  </div>
                  {(()=>{
                    const mGroups=(()=>{
                    ...
                    return mGroups.length===0
                      ?<div style={{textAlign:"center",color:C.muted,padding:"16px 0",fontSize:13}}>本月尚無記錄</div>
                      :mGroups.map(({groupId,lead})=>{
                        const isOut=["withdraw_out","transfer_out"].includes(lead.type);
                        const isIn=["withdraw_in","transfer_in"].includes(lead.type);
                        const isSavDep=lead.type==="savings_deposit";
                        const isRedeem=lead.type==="goal_redeem";
                        ...
                        return(
                          <div key={groupId}
                            style={{display:"flex",alignItems:"center",gap:8,
                              ...
                              background:lp===groupId?"#fce8e0":C.card,transition:"background .1s"}}
                            onMouseDown={()=>startLP(groupId)} onMouseUp={cancelLP} onMouseLeave={cancelLP}
                            onTouchStart={()=>startLP(groupId)} onTouchEnd={cancelLP}>
                            {iconEl?<div style={S.icon(iconColor,34)}>{iconEl}</div>:<div style={S.iconT(iconColor,34)}>{(cat?.name||"?").slice(0,2)}</div>}
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontWeight:600,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isSavDep?"儲蓄":lead.note||cat?.name}</div>
                              <div style={{fontSize:11,color:C.muted}}>{lead.date} · {acc.icon} {acc.name}</div>
                            </div>
                  ...
                  {/* Pie charts */}
                  {calPieInc.length>0&&(
                    <div style={{background:C.card,borderRadius:14,padding:"14px",border:`1px solid ${C.border}`,marginBottom:10,marginTop:8}}>
